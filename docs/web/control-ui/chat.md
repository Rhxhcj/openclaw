...
revisit saved locations in the current conversation. The position rail sits on
the left; bookmarked points use the same claw. Nearby bookmarks share a marker:
...
personal settings, still applies. A full quota produces a visible error rather
than discarding another favorite. Reopening the list or reconnecting refreshes
